...
``` markdown
<metric_name>{<label_matcher_list>, ar_indexbase=<ar_idnexbase>}
```

+ 必要参数:
  + `<metric_name>`: 需要查询的指标名，指标名称是存储在索引库中的一个字段，根据日志结构定义。
  + `<ar_indexbase>`: 指定所查询的指标名所在的索引库，可指定多个索引库，多个索引库之间用英文逗号隔开。
...
以下示例查询的是仅选择具有 `metrics.system_metric_memory_total_bytes` 指标名称且 `labels.instance` 标签设置为 `0.0.0.0` 且数据存储在索引库 `system_metric` 下的时间序列。

``` markdown
metrics.system_metric_memory_total_bytes{ar_indexbase="system_metric",labels.instance="0.0.0.0"}
```

标签匹配也可以通过与内部标签匹配来应用于指标名称 `__name__`。例如，表达式`metrics.system_metric_memory_total_bytes` 等价于 `{__name__="metrics.system_metric_memory_total_bytes"}`。
...
范围向量选择器选择的是当前时间回溯一段时间内的所有样本点。我们当前的 PromQL 尚未支持范围向量选择器的单独使用，应结合 `irate` ~~或者 `rate`~~ 函数使用。

2. 语法结构
``` markdown
<metric_name>{<label_matcher_list>, ar_indexbase=<ar_idnexbase>}[time_durations]
```
...
``` markdown
irate(metrics.http_requests_total{ar_indexbase="test",labels.job="api-server"}[5m])
```
~~与 rate 结合使用~~

~~以下示例表达式返回针对范围向量中过去 5 分钟内的所有数据点的 HTTP 请求的每秒平均增长速率：~~

~~rate(metrics.http_requests_total{ar_indexbase="test",labels.job="api-server"}[5m])~~



### 2.3.3 操作符

PromQL 的操作符包含算术二元运算符、一对一匹配的向量匹配、聚合算子。
...
``` markdown
metrics.system_metric_memory_total_bytes{labels.instance="0.0.0.0",ar_indexbase="system_metric"} / 1024
```
...
``` markdown
nodmetrics.system_metric_memory_total_bytese_memory_MemTotal_bytes{labels.instance="0.0.0.0",ar_indexbase="system_metric"} - metrics.system_metric_memory_available_bytes{labels.instance="0.0.0.0",ar_indexbase="system_metric"}
```
...
+ on 示例查询
``` markdown
 http_errors{code="500", ar_indexbase="test"} / on(method) http_requests{ar_indexbase="test"}
```
+ on 示例查询结果
``` markdown
...
+ ignoring 示例查询
``` markdown
 http_errors{name="500", ar_indexbase="test"} / ignoring(code) http_requests{ar_indexbase="test"}
```
+ ignoring 示例查询结果
``` markdown
{method="get"}  0.04            //  24 / 600
...
+ sum 示例
``` markdown
sum(http_errors{ar_indexbase="test"}) without(method)
```

等价于：
``` markdown
sum(http_errors{ar_indexbase="test"}) by(code)
```
...
``` markdown
sum(http_errors{ar_indexbase="test"})
```
...
~~topk(3, http_errors{ar_indexbase="test"})~~
...
### 2.3.4 函数
PromQL 提供的内置函数有： time(), irate ~~, rate, sort, sort_desc, label_replace, label_join~~.
...
~~p{host="localhost:9100---node",instance="localhost:9100",job="node"} 1~~
...
{ "@timestamp" : "2022-04-25T09:46:00.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 11,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "2" } }
{ "@timestamp" : "2022-04-25T09:46:30.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 12,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "3" } }
{ "@timestamp" : "2022-04-25T09:47:00.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 12,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "4" } }
{ "@timestamp" : "2022-04-25T09:47:30.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 13,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "5" } }
{ "@timestamp" : "2022-04-25T09:48:00.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 14,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "6" } }
{ "@timestamp" : "2022-04-25T09:48:30.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 15,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "7" } }
{ "@timestamp" : "2022-04-25T09:46:00.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 21,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "8" } }
{ "@timestamp" : "2022-04-25T09:46:30.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 22,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "9" } }
{ "@timestamp" : "2022-04-25T09:47:00.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 22,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "10" } }
{ "@timestamp" : "2022-04-25T09:47:30.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 23,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "11" } }
{ "@timestamp" : "2022-04-25T09:48:00.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 24,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "12" } }
{ "@timestamp" : "2022-04-25T09:48:30.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=user","metrics.system_metric_cpu_seconds_total": 25,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "user" }
{ "index" : { "_index" : "promql_test-01", "_id" : "13" } }
{ "@timestamp" : "2022-04-25T09:46:00.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 31,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "14" } }
{ "@timestamp" : "2022-04-25T09:46:30.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 32,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "15" } }
{ "@timestamp" : "2022-04-25T09:47:00.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 32,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "16" } }
{ "@timestamp" : "2022-04-25T09:47:30.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 33,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "17" } }
{ "@timestamp" : "2022-04-25T09:48:00.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 34,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "18" } }
{ "@timestamp" : "2022-04-25T09:48:30.000Z", "labels_str" : "labels.cpu=cpu0,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 35,"labels.cpu": "cpu0","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "19" } }
{ "@timestamp" : "2022-04-25T09:46:00.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 41,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "20" } }
{ "@timestamp" : "2022-04-25T09:46:30.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 42,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "21" } }
{ "@timestamp" : "2022-04-25T09:47:00.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 42,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "22" } }
{ "@timestamp" : "2022-04-25T09:47:30.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 43,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "23" } }
{ "@timestamp" : "2022-04-25T09:48:00.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 44,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "system" }
{ "index" : { "_index" : "promql_test-01", "_id" : "24" } }
{ "@timestamp" : "2022-04-25T09:48:30.000Z", "labels_str" : "labels.cpu=cpu1,labels.instance=10.4.14.35,labels.mode=system","metrics.system_metric_cpu_seconds_total": 45,"labels.cpu": "cpu1","labels.instance": "10.4.106.127","labels.mode": "system" }
'
```
...
-H "Content-Type:application/x-www-form-urlencoded" \
-d 'query=count(metrics.system_metric_cpu_seconds_total{labels.mode="system",labels.instance="10.4.106.127",ar_indexbase="promql_test"})
 &time=2022-04-25T09:48:30.000Z'
```
...
-H "Content-Type:application/x-www-form-urlencoded" \
-d 'query=irate(metrics.system_metric_cpu_seconds_total{labels.mode="system",labels.instance="10.4.106.127",ar_indexbase="promql_test"}[1m])
 &start=2022-04-25T09:46:00.000Z&end=2022-04-25T09:48:30.000Z&step=30s'
```
...
一个简单的指标，例如 `metrics.system_metric_memory_total_bytes` 可以扩展到数千个具有不同标签的时间序列。所以，即使输出只是少数时间序列，聚合多个时间序列的语句也会在服务器上产生负载。这类似于在关系数据库中对列的所有值求和会很慢，即使输出值只是一个数字。
